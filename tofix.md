...
- [x] infra 容器时，io也不需要建立。
- [x] Xen vcpu reader
- [x] 验证xl vcpu-list相关输出和micran存储的都同步！后者保证性能，不应该总是从xl输出来读取信息
...
- [x] container update not implemented: recalculate resource, realloca resources; 资源伸缩的真实实现
- [ ] CNI support
...
- [ ] 真正的 waitContainerExit: 需要真正可用的micad client status monitor
