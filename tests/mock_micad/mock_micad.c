...
static int create_mock_pty(const char *pty_path)
{
    // Try to remove existing file first
    unlink(pty_path);
    
    // Create a regular file instead of a device node
    FILE *pty = fopen(pty_path, "w");
    if (!pty) {
        perror("Failed to create mock PTY file");
        WARN("Failed to create mock PTY file: %s", pty_path);
        return -1;
    }
    ...
    if (chmod(pty_path, 0666) < 0) {
        perror("Failed to set permissions on mock PTY");
        WARN("Failed to set permissions on mock PTY: %s", pty_path);
    }
...
static void cleanup_mock_pty(const char *pty_path)
{
    if (unlink(pty_path) < 0 && errno != ENOENT) {
        perror("Failed to remove mock PTY");
        WARN("Failed to remove mock PTY file: %s", pty_path);
    } else if (errno != ENOENT) {
        INFO("Mock PTY file cleaned up: %s\n", pty_path);
    }
}
...
static void remove_client(const char *name)
{
    ...
    char pty_path[128];
    snprintf(pty_path, sizeof(pty_path), "/tmp/ttyRPMSG%s", name);
    cleanup_mock_pty(pty_path);
}
...
static int remove_socket(const char *client_name)
{
    ...
    if (unlink(socket_path) < 0 && errno != ENOENT) {
        perror("Failed to remove socket");
        return -1;
    }
...
static int create_client_socket(const char *client_name)
{
    ...
    /* check socket_path exists and is of a socket type */
    struct stat st;
    if (stat(socket_path, &st) == 0) {
...
static void handle_client(int client_fd)
{
    ...
    if (bytes_received == sizeof(struct create_msg)) {
        ...
        if (create_ret == 0) {
            ...
            printf("Successfully added client<%s> with client socket and mock PTY\n", client_name);
            if (send_response) {
                safe_send(client_fd, RESPONSE_SUCCESS, strlen(RESPONSE_SUCCESS));
            }
        } else {
